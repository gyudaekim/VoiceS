...
This guide provides detailed instructions for building VoiceS from source.
...
The easiest way to build VoiceS is using the included Makefile, which automates the entire build process including building and linking the whisper framework.

### Simple Build Commands

```bash
# Clone the repository
git clone https://github.com/gdkim/VoiceS.git
cd VoiceS
...
- `make build` - Build the VoiceS Xcode project
- `make local` - Build for local use (no Apple Developer certificate needed)
- `make run` - Launch the built VoiceS app
...
1. **Manages Dependencies**: Creates a dedicated `~/VoiceS-Dependencies` directory for all external frameworks
...
This builds VoiceS with ad-hoc signing using a separate build configuration (`LocalBuild.xcconfig`) that requires no Apple Developer account.

### How It Works

The `make local` command uses:
- `LocalBuild.xcconfig` to override signing and entitlements settings
- `VoiceS.local.entitlements` (stripped-down, no CloudKit/keychain groups)
...
For more help, please check the [issues](https://github.com/gdkim/VoiceS/issues) section or create a new issue. 