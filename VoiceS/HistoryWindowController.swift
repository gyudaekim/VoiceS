...
class HistoryWindowController: NSObject, NSWindowDelegate {
    ...
    private let windowIdentifier = NSUserInterfaceItemIdentifier("com.gdkim.voices.historyWindow")
    private let windowAutosaveName = NSWindow.FrameAutosaveName("VoiceSHistoryWindowFrame")

    private override init() {
        super.init()
    }

    func showHistoryWindow(modelContainer: ModelContainer, engine: VoiceSEngine) {
    ...
    private func createHistoryWindow(modelContainer: ModelContainer, engine: VoiceSEngine) -> NSWindow {
        ...
        window.title = "VoiceS — Transcription History"
